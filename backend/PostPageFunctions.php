...
function rid_addToPost($ridPid, $ridPcontent, $ridTitle, $posttype, $postriddle, $rid) {
    ...
    if ($posttype == 'wp_riddlepost_prev') {
        ...
        if ($arr == NULL) {
            ...
            $arr[$c]['data_game'] = $postriddle["data_game"];
            $arr[$c]['data_height'] = $postriddle["data-height"];
            $arr[$c]['data_width'] = $postriddle["data-width"];
            $arr[$c]['data_id'] = $rid;
            update_metadata('post', $ridPid, 'metaRid1', $arr);
        }
    }
...
function rid_updatePost($ridPid, $ridPcontent, $posttype, $postriddle, $ridFrameCode) {
    ...
    if ($posttype == 'wp_riddlepost_prev') {
        ...
        for ($b = 0; is_array($arr_new) && $b < count($arr_new); $b++) {
             $ridFrameCode = rid_createFrameCode($arr_new[$b]['src'], $arr_new[$b]['width'], $arr_new[$b]['height']);
...
function rid_createFrameCode($rid_src, $ridwidth, $ridheight) {
    ...
    $riddleFrameCode = array(
        ...
        'iFrameCode' => '<div class="riddle_list"  data-width="' . $ridwidth . '" data-height="' . $ridwidth . '" data-game="' . $rid_src . '" ></div> <script type="text/javascript" src="//www.riddle.com/files/js/embed.js?qvs=2.1"></script>', //'<iframe  width="100%" height="255px" src="'.$rid_src.'" ohref="/Embed/List/preview" style="border: medium none; width: '.$ridwidth.'; height: '.$ridheight.';" border="0"> ',
        'ridWidth' => $ridwidth,
        'ridHeight' => $ridheight
    );
